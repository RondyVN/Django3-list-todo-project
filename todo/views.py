    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo/signupuser.html', {'form': UserCreationForm(), 'error': 'That usernam'})
        else:
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo/login.html', {'form': AuthenticationForm(), 'error': 'Username and password did not mach'})
        else:
            login(request, user)
            return redirect('currenttodos')
...
@login_required
def currenttetodo(request, todo_pk):
    todo = get_object_or_404(Todo, pk=todo_pk, user=request.user)
    if request.method == 'POST':
